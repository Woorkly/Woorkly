...
function BadgeRole({ role }) {
  return (
    <span className={`badge ${role === "Admin" ? "b-confirm" : "b-role-user"}`}>{role}</span>
  );
}

function BadgeStatut({ statut }) {
  return (
    <span className={`badge ${statut === "Actif" ? "b-available" : "b-offline"}`}>{statut}</span>
  );
}
...
function Avatar({ initiales, couleur, size = 36 }) {
  return (
    <div className="u-avatar" style={{ width: size, height: size, background: couleur, fontSize: size * 0.35 }}>
      {initiales}
    </div>
  );
}
    ...
    <div className="ud-overlay" onClick={onClose}>
      <div className="ud-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="ud-section">
          ...
          {user.reservations.length === 0 ? (
            ...
            <table className="data-table">
              <thead>
                <tr><th>Date</th><th>Salle</th><th>Statut</th></tr>
              </thead>
              <tbody>
                {user.reservations.map((r, i) => (
                  <tr key={i}>
                    <td>{r.date}</td>
                    <td>{r.salle}</td>
                    <td><BadgeResa statut={r.statut} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="ud-actions">
          <button className="btn-primary" style={{ fontSize: "0.82rem", display:"flex", alignItems:"center", gap:"0.4rem" }}><IconEdit /> Modifier</button>
          <button className="ud-btn-danger" style={{ display:"flex", alignItems:"center", gap:"0.4rem" }}><IconTrash /> Supprimer</button>
        ...
        <div className="page-body">
          ...
          <div className="kpi-row">
            <div className="kpi-card c-blue">
              ...
              <div className="kpi-row-val">
                <span className="kpi-val">{usersData.length}<span className="kpi-unit"> users</span></span>
              </div>
            </div>
            <div className="kpi-card c-green">
              <p className="kpi-label">Actifs</p>
              <div className="kpi-row-val">
                <span className="kpi-val">{usersData.filter((u) => u.statut === "Actif").length}<span className="kpi-unit"> users</span></span>
              </div>
            </div>
            <div className="kpi-card c-orange">
              <p className="kpi-label">Inactifs</p>
              <div className="kpi-row-val">
                <span className="kpi-val">{usersData.filter((u) => u.statut === "Inactif").length}<span className="kpi-unit"> users</span></span>
              </div>
            </div>
            <div className="kpi-card c-teal">
              <p className="kpi-label">Admins</p>
              <div className="kpi-row-val">
                <span className="kpi-val">{usersData.filter((u) => u.role === "Admin").length}<span className="kpi-unit"> admins</span></span>
              </div>
            </div>
          </div>
          ...
          <div className="filters-row">
            <div className="search-wrap">
              ...
              <input className="search-box" placeholder="Rechercher un utilisateur" value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
            <span className="flt-label">Filtres :</span>
            <select className="flt-select" value={roleFilter} onChange={(e) => setRoleFilter(e.target.value)}>
              <option value="">Rôle ▾</option>
              <option value="Admin">Admin</option>
              <option value="Utilisateur">Utilisateur</option>
            </select>
            <select className="flt-select" value={statFilter} onChange={(e) => setStatFilter(e.target.value)}>
              ...
              <tbody>
                {filtered.map((u) => (
                  <tr key={u.id} className="u-row" onClick={() => setSelected(u)}>
                    <td><Avatar initiales={u.initiales} couleur={u.couleur} /></td>
                    <td style={{ fontWeight: 500 }}>{u.nom}</td>
                    <td style={{ color: "var(--muted)", fontSize: "0.8rem" }}>{u.email}</td>
                    <td><BadgeRole role={u.role} /></td>
                    <td><BadgeStatut statut={u.statut} /></td>
                    <td style={{ color: "var(--muted)", fontSize: "0.8rem" }}>{u.inscription}</td>
                    <td>
                      <span className="u-resa-count">{u.reservations.length}</span>
                    </td>
                    <td onClick={(e) => e.stopPropagation()}>
                      <button className="act-btn act-view"  onClick={() => setSelected(u)}><IconEye /></button>
                      <button className="act-btn act-edit"><IconEdit /></button>
                      <button className="act-btn act-del"><IconTrash /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
